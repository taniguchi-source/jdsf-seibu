...
出力: preftest/audio/manual/scene-00.mp3 〜 scene-24.mp3
...
NARRATIONS = [
    ...
    "公式SNSリンクでは、トップページのメインビジュアルに、フェイスブック、インスタグラム、ライン、ユーチューブなどの公式SNSアイコンを表示できます。使いたいSNSのアドレスを入れて保存すると、入力した分だけアイコンが表示され、空欄のものは表示されません。また、ニュースティッカー管理の、横に流すにチェックすると、上部のニューバーのニュースが、横に流れるスクロール表示になります。オフのままなら、これまで通り折り返して全文を表示します。",
