...
出力: preftest/audio/manual/scene-00.mp3 〜 scene-25.mp3
...
NARRATIONS = [
    ...
    "ナビゲーション管理で、上部メニューに並ぶページの名前や表示・非表示を決めます。ページ名を分かりやすく変え、ページ設定を保存を押します。そのページの中身は、下のコンテンツ管理のプラス追加で作ります。",
    ...
    "連盟が別のホームページを持っている場合は、表示方法で見せ方を選べます。このサイトのページなら、コンテンツ管理で作った中身が出ます。別のサイトを埋め込むを選ぶと、そのサイトの中身がこのページの中に表示され、ヒーローと上部メニューはそのまま残ります。別のサイトへ移動を選ぶと、押したときにそのサイトへ出ていきます。埋め込むときは、URLと高さを入れて、見え方を確認を押すと、その高さでの見え方をその場で確かめられます。ちょうどよくなったら、ページ設定を保存を押します。",
    "コンテンツ管理の、プラス追加で部品を作ります。部品は7種類。テキスト、スプレッドシートの表、画像、ギャラリー、PDF、リンク、サイト埋め込みから選びます。左の点のマークをつかむと並べ替えができます。",
    ...
    "クイックリンクは、トップページ右側に並ぶボタンです。表示名とリンク先を設定し、必要なら文字の代わりに画像をボタンとして登録できます。画像は、並びが乱れないよう、すべて同じ大きさの横長ボタンに引き伸ばして表示されます。そのため、画像は横長で用意してください。縦長のポスターをそのまま入れると、横に潰れて見えます。使わない行は今まで通り文字ボタンになります。",
